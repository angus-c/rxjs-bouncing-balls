...
var xVelocities = xChange
  .map(function(data) {
    return {ball: data.ball, vel: newVelocity(data, 'xVel')}});

Rx.Observable.merge(xVelocities, xOpposite)
  .subscribe(function (data) {
    data.ball.xVel = data.vel;
  });

var yVelocities = yChange
  .map(function(data) {
    return {ball: data.ball, vel: newVelocity(data, 'yVel')}});

Rx.Observable.merge(yVelocities, yOpposite)
  .subscribe(function (data) {
...
function newVelocity(data, dimension) {
  if (classForSVG(data.obstacle) == 'ball') {
    ...
    opposingStream.onNext(
      {ball: data.obstacle.ball, vel: -Math.abs(data.ball[dimension])});
    // then return this ball's new velocity
    return Math.abs(data.obstacle.ball[dimension]);
  } else {
