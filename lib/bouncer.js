...
var xOpposite = new Rx.Subject();
var yOpposite = new Rx.Subject();


xChange
  .map(function(data) {
    return {ball: data.ball, vel: newVelocity(data, 'xVel')}})
  .merge(xOpposite)
  .subscribe(function (data) {
    data.ball.xVel = data.vel;
  });

yChange
  .map(function(data) {
    return {ball: data.ball, vel: newVelocity(data, 'yVel')}})
  //.merge(yOpposite)
  .subscribe(function (data) {
...
function newVelocity(data, dimension) {
  if (classForSVG(data.obstacle) == 'ball') {
    // another ball - naive conservation of momentum, assume no spin
    // only check left and top hits so we don't check same collision twice
    if (data.direction > 0) {
      // trigger other ball's velocity change
      var opposingStream = (dimension == 'xVel') ? xOpposite : yOpposite;
      opposingStream.onNext({ball: data.obstacle.ball, vel: data.ball[dimension]});
      // return this ball's new velocity
      return data.direction * Math.abs(data.obstacle.ball[dimension]);
    }
  } else {
    // wall or other stationary obstacle - reverse velocity
    return data.direction * Math.abs(data.ball[dimension]);
  }
}
...
Ball.prototype.checkCollision = function () {
  for (var i = 0; i < probes.length; i++) {
    ...
    if (probes[i][0]) {
        ...
        xChange.onNext({ball: detected, obstacle: this, direction: -newDirection});

    }
    if (probes[i][1]) {
        var newDirection = (probes[i][1] < 0) ? 1 : -1;
        yChange.onNext({ball: this, obstacle: detected, direction: newDirection});
        yChange.onNext({ball: detected, obstacle: this, direction: -newDirection});
    }
  }
};
